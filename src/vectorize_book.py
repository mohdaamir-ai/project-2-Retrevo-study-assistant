...
def vectorize_book_and_store_to_db(class_subject_name , vector_db_name ):
    ...
    loader = DirectoryLoader(path=book_dir,glob="./*.pdf",loader_cls=PyMuPDFLoader)
    documents = loader.load()
    text_chunk = text_splitter.split_documents(documents)
    Chroma.from_documents(documents=text_chunk,embedding=embedding,persist_directory=vector_db_path)
    print(f"{class_subject_name} saved to vector db: {vector_db_name}")
    return 0

def vectorize_chapter(class_subject_name):
    book_dir = f"{data_dir}/{class_subject_name}"
